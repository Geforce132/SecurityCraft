...
public class BlockLaserBlock extends BlockOwnable {
	...
	public static void destroyAdjacentLasers(World world, int x, int y, int z){
		for(int i = 1; i <= SecurityCraft.config.laserBlockRange; i++){
			...
			if(id == SCContent.laserBlock){
				for(int j = 1; j < i; j++)
					if(BlockUtils.getBlock(world, x + j, y, z) == SCContent.laserField && (Integer)world.getBlockState(new BlockPos(x + j, y, z)).getValue(BlockLaserField.BOUNDTYPE) == 3)
		...
		for(int i = 1; i <= SecurityCraft.config.laserBlockRange; i++){
			...
			if(id == SCContent.laserBlock){
				for(int j = 1; j < i; j++)
					if(BlockUtils.getBlock(world, x, y, z + j) == SCContent.laserField && (Integer)world.getBlockState(new BlockPos(x, y, z + j)).getValue(BlockLaserField.BOUNDTYPE) == 2)
		...
		for(int i = 1; i <= SecurityCraft.config.laserBlockRange; i++){
			...
			if(id == SCContent.laserBlock){
				for(int j = 1; j < i; j++)
					if(BlockUtils.getBlock(world, x, y + j, z) == SCContent.laserField && (Integer)world.getBlockState(new BlockPos(x, y + j, z)).getValue(BlockLaserField.BOUNDTYPE) == 1)
