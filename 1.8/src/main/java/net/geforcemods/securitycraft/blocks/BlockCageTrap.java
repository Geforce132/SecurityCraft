	...
	public void onEntityIntersected(World world, BlockPos pos, Entity entity) {
		if(!world.isRemote){
			...
			if(isPlayer || (entity instanceof EntityMob && shouldCaptureMobs)){
				...
				if(isPlayer)
					MinecraftServer.getServer().getConfigurationManager().sendChatMsg(new ChatComponentTranslation("["+ EnumChatFormatting.BLACK + StatCollector.translateToLocal("tile.securitycraft:cageTrap.name") + EnumChatFormatting.RESET + "] " + StatCollector.translateToLocal("messages.securitycraft:cageTrap.captured").replace("#player", ((EntityPlayer) entity).getCommandSenderName()).replace("#location", Utils.getFormattedCoordinates(pos))));
			}
		}
	}
