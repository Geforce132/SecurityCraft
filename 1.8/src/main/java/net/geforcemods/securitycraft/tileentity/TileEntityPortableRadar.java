	...
	public boolean attackEntity(Entity attacked) {
		if (attacked instanceof EntityPlayer)
		{
			...
			if(entities.isEmpty())
			{
				...
				if(!redstoneModule || (redstoneModule && ((Boolean)worldObj.getBlockState(pos).getValue(BlockPortableRadar.POWERED)).booleanValue()))
				{
			...
			if(owner != null && hasModule(EnumCustomModules.WHITELIST) && ModuleUtils.getPlayersFromModule(worldObj, pos, EnumCustomModules.WHITELIST).contains(attacked.getCommandSenderName().toLowerCase()))
				return false;
