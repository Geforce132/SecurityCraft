...
public class GuiKeypadFurnaceSetup extends GuiContainer
{
    private static final ResourceLocation field_110410_t = new ResourceLocation("securitycraft:textures/gui/container/blank.png");
	private TileEntityKeypadFurnace keypadInventory;
	...
	public GuiKeypadFurnaceSetup(InventoryPlayer par1InventoryPlayer, TileEntityKeypadFurnace par2TileEntityKeypadChest)
    {
        super(new ContainerGeneric(par1InventoryPlayer, par2TileEntityKeypadChest));
    ...
    public void initGui(){
		...
		this.textboxKeycode = new GuiTextField(1, this.fontRendererObj, this.width / 2 - 37, this.height / 2 - 47, 77, 12);
	...
	protected void actionPerformed(GuiButton guibutton){
		...
		case 0:
			try{							
				mod_SecurityCraft.network.sendToServer(new PacketSetKeypadCode(keypadInventory.getPos().getX(), keypadInventory.getPos().getY(), keypadInventory.getPos().getZ(), this.textboxKeycode.getText()));						
			}catch(Exception e){
				this.flag  = true;
				this.updateButtonText();
				e.printStackTrace();
			...
			Minecraft.getMinecraft().thePlayer.openGui(mod_SecurityCraft.instance, 15, Minecraft.getMinecraft().theWorld, keypadInventory.getPos().getX(), keypadInventory.getPos().getY(), keypadInventory.getPos().getZ());
