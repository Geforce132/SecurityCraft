  	  	...
  	  	if(!par3World.isRemote){  	  	
  	  		if(par3World.getBlockState(pos).getBlock() instanceof IExplosive){ //isValidMine(par3World, pos)
  	  			if(!isMineAdded(par1ItemStack, par3World, pos)){
		  	  		...
		  	  		PlayerUtils.sendMessageToPlayer(par2EntityPlayer, par2EntityPlayer.getName() + " bound a mine at X:" + pos.getX() + " Y:" + pos.getY() + " Z:" + pos.getZ() + " to a remote access tool.", null);
  	  			}else{
  	  				this.removeTagFromItemAndUpdate(par1ItemStack, pos, par2EntityPlayer);
  	  				PlayerUtils.sendMessageToPlayer(par2EntityPlayer, par2EntityPlayer.getName() + " unbound a mine at X:" + pos.getX() + " Y:" + pos.getY() + " Z:" + pos.getZ() + " from a remote access tool.", null);
  	  			}
  	  		}else{
