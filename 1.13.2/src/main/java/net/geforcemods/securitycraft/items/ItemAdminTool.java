		...
		if(!world.isRemote && CommonConfig.CONFIG.allowAdminTool.get()) {
			if(world.getTileEntity(pos) != null) {
				...
				if(te instanceof TileEntitySecretSign)
				{
					...
					for(int i = 0; i < 4; i++)
					{
						PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.adminTool.getTranslationKey()), ((TileEntitySecretSign)te).signText[i].getUnformattedComponentText(), TextFormatting.DARK_PURPLE);
					}
