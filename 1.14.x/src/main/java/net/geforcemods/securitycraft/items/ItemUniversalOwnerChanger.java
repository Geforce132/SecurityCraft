...
public class ItemUniversalOwnerChanger extends Item
{
	...
	public ActionResultType onItemUse(PlayerEntity player, World world, BlockPos pos, ItemStack stack, Direction side, double hitX, double hitY, double hitZ)
	{
		...
		if(!world.isRemote)
		{
			...
			if(isDefault)
			{
				if(CommonConfig.CONFIG.allowBlockClaim.get())
			...
			world.getServer().func_184103_al().sendPacketToAllPlayers(te.getUpdatePacket());

			if(door)
				world.getServer().func_184103_al().sendPacketToAllPlayers(((TileEntityOwnable)world.getTileEntity(updateTop ? pos.up() : pos.down())).getUpdatePacket());
