...
public class EntityIMSBomb extends EntityFireball {
	...
	public void setTarget() {
		if(playerName != null && PlayerUtils.isPlayerOnline(playerName)){
			...
			WorldUtils.addScheduledTask(worldObj, () -> worldObj.spawnEntityInWorld(imsBomb));
			setDead();
	...
	protected void onImpact(RayTraceResult result){
		if(!worldObj.isRemote)
			if(result.typeOfHit == Type.BLOCK && BlockUtils.getBlock(worldObj, result.getBlockPos()) != SCContent.ims){
				worldObj.createExplosion(this, result.getBlockPos().getX(), result.getBlockPos().getY() + 1D, result.getBlockPos().getZ(), 7F, true);
				setDead();
			}
	}
