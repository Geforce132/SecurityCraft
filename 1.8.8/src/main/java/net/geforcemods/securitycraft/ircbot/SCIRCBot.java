		...
		for(User user: getUsers(channel))
			if(channel.matches("#GeforceMods") && (user.hasVoice() || user.isOp()) && (message.startsWith((getNick() + ":")) || message.startsWith((getNick() + ",")))){
				...
				if(message.split(" ")[1].equals(prefix + "info"))
				{
					...
					sendMessageToPlayer(EnumChatFormatting.YELLOW + "<" + sender + " (IRC) --> " + getNick().replace("SCUser_", "") + "> " + EnumChatFormatting.RESET + (message.startsWith(getNick() + ":") ? message.replace(getNick() + ":", "") : message.replace(getNick() + ",", "")), getPlayer());
