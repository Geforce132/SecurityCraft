...
public class GuiKeypadFurnaceInventory extends GuiContainer{
	...
	{
		String s = gurnace ? "Keypad Gurnace" : (tileFurnace.hasCustomName() ? tileFurnace.getCommandSenderName() : I18n.format(StatCollector.translateToLocal("gui.protectedFurnace.name"), new Object[0]));
	...
	public void onGuiClosed(){
		super.onGuiClosed();
		mod_SecurityCraft.network.sendToServer(new PacketSetBlock(tileFurnace.getPos().getX(), tileFurnace.getPos().getY(), tileFurnace.getPos().getZ(), "securitycraft:keypadFurnace", mc.theWorld.getBlockState(tileFurnace.getPos()).getBlock().getMetaFromState(mc.theWorld.getBlockState(tileFurnace.getPos())) - 6));
	}
