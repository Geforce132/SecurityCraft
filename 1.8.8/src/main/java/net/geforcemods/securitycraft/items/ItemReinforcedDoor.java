    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float par8, float par9, float par10)
    {
    	if(world.isRemote){
	        ...
	        else
	        {
	            ...
	            {
	                placeDoor(world, pos, EnumFacing.fromAngle((double)player.rotationYaw), mod_SecurityCraft.reinforcedDoor);                    //TERD.setOwner(player.getGameProfile().getId().toString(), player.getName());
