...
public class GuiCameraMonitorOverlay extends GuiScreen {
	...
	protected void actionPerformed(GuiButton guibutton){
		mod_SecurityCraft.network.sendToServer(new PacketMountCamera(cameraMonitor.getCameraPositions(nbtTag).get(guibutton.id)[0], cameraMonitor.getCameraPositions(nbtTag).get(guibutton.id)[1], cameraMonitor.getCameraPositions(nbtTag).get(guibutton.id)[2]));
	}
