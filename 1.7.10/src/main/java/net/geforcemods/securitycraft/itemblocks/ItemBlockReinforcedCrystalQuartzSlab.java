...
public class ItemBlockReinforcedCrystalQuartzSlab extends ItemBlock
{
	...
	public ItemBlockReinforcedCrystalQuartzSlab(Block blockType, Boolean notSlab)
	{
		super(blockType);
		singleSlab = (BlockSlab)SCContent.reinforcedCrystalQuartzSlab;
		isNotSlab = notSlab;
		setMaxDurability(0);
		setHasSubtypes(true);
	}
	...
	{
		return SCContent.reinforcedCrystalQuartz.getIcon(2, meta);
	}
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		else
		{
			...
			{
				if(world.checkNoEntityCollision(this.getBlockVariant(meta).getCollisionBoundingBoxFromPool(world, x, y, z)) && world.setBlock(x, y, z, this.getBlockVariant(block, meta), (block == SCContent.reinforcedCrystalQuartzSlab && meta == 2 ? 2 : blockType), 3))
				{
	...
	{
		return SCContent.reinforcedDoubleCrystalQuartzSlab;
	}

	public Block getBlockVariant(int meta)
	{
		return Block.getBlockFromItem(new ItemStack(SCContent.reinforcedCrystalQuartzSlab, 1, meta).getItem());
	}

	public boolean isBlock(Block block)
	{
		return block == SCContent.reinforcedCrystalQuartzSlab;
	}
}
