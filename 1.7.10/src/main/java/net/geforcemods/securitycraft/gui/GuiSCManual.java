...
public class GuiSCManual extends GuiScreen {
	...
	protected void actionPerformed(GuiButton button){
		...
		((GuiButton)buttonList.get(2)).visible = currentPage != -1 && subpages.size() > 0;
		((GuiButton)buttonList.get(3)).visible = currentPage != -1 && subpages.size() > 0;
	}
	...
	public void handleMouseInput()
	{
		...
		//hide subpage buttons on main page
		((GuiButton)buttonList.get(2)).visible = currentPage != -1 && subpages.size() > 1;
		((GuiButton)buttonList.get(3)).visible = currentPage != -1 && subpages.size() > 1;
	}
	...
	static class ChangePageButton extends GuiButton {
		...
		public ChangePageButton(int id, int x, int y, boolean forward){
			super(id, x, y, 23, 13, "");
			isForward = forward;
		}
