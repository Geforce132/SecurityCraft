...
public class GuiMRATDetonate extends GuiContainer{
	...
	public void initGui(){
		...
		for(int i = 1; i < 7; i++){
			...
			if(item.getItem() != null && item.getItem() == SCContent.remoteAccessMine && item.stackTagCompound != null &&  item.stackTagCompound.getIntArray("mine" + i) != null && item.stackTagCompound.getIntArray("mine" + i).length > 0){
				...
				buttons[i - 1].enabled = (mc.theWorld.getBlock(coords[0], coords[1], coords[2]) instanceof IExplosive && (!((IExplosive) mc.theWorld.getBlock(coords[0], coords[1], coords[2])).isDefusable() || ((IExplosive) mc.theWorld.getBlock(coords[0], coords[1], coords[2])).isActive(Minecraft.getMinecraft().theWorld, coords[0], coords[1], coords[2]))) ? true : false;
				buttons[i - 1].id = i - 1;
			}
	...
	protected void actionPerformed(GuiButton guibutton){
		...
		removeTagFromItemAndUpdate(item, coords[0], coords[1], coords[2], Minecraft.getMinecraft().thePlayer);
	}
