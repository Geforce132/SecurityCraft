...
public class GuiCameraMonitor extends GuiContainer {
	...
	public void initGui(){
		...
		for(int i = 0; i < 10; i++) {
			...
			if((view = views.get(camID - 1)) != null) {
				...
				if(Minecraft.getMinecraft().theWorld.getBlock(view.x, view.y, view.z) != SCContent.securityCamera) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for(int i = 0; i < hoverCheckers.length; i++)
			if(hoverCheckers[i] != null && hoverCheckers[i].checkHover(mouseX, mouseY)){
				if(cameraTEs[i] == null)
					drawHoveringText(mc.fontRendererObj.listFormattedStringToWidth(StatCollector.translateToLocal("gui.securitycraft:monitor.cameraInDifferentDim").replace("#", cameraViewDim[i] + ""), 150), mouseX, mouseY, mc.fontRendererObj);

				if(cameraTEs[i] != null && cameraTEs[i].hasCustomName())
					drawHoveringText(mc.fontRendererObj.listFormattedStringToWidth(StatCollector.translateToLocal("gui.securitycraft:monitor.cameraName").replace("#", cameraTEs[i].getCustomName()), 150), mouseX, mouseY, mc.fontRendererObj);
			}
	}
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		fontRendererObj.drawString(StatCollector.translateToLocal("gui.securitycraft:monitor.selectCameras"), xSize / 2 - fontRendererObj.getStringWidth(StatCollector.translateToLocal("gui.securitycraft:monitor.selectCameras")) / 2, 6, 4210752);
	}
