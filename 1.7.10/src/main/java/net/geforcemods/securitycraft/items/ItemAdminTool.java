		...
		if(!world.isRemote && SecurityCraft.config.allowAdminTool) {
			if(world.getTileEntity(x, y, z) != null) {
				...
				if(te instanceof TileEntitySecretSign)
				{
					...
					for(int i = 0; i < 4; i++)
					{
						PlayerUtils.sendMessageToPlayer(player, StatCollector.translateToLocal("item.securitycraft:adminTool.name"), ((TileEntitySecretSign)te).signText[i], EnumChatFormatting.DARK_PURPLE);
					}
