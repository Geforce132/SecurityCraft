...
public class EntitySentry extends EntityCreature implements IRangedAttackMob //needs to be a creature so it can target a player, ai is also only given to living entities
{
	...
	public void setAttackTarget(EntityLivingBase target)
	{
		if(getMode() != EnumSentryMode.AGGRESSIVE && (target == null && previousTargetId != Long.MIN_VALUE || (target != null && previousTargetId != target.getEntityId())))
		{
			...
			animate = true;
			SecurityCraft.network.sendToAll(new PacketCInitSentryAnimation(posX, posY, posZ, animate, animateUpwards));
		}
	...
	public void setDisguiseModule(ItemStack module)
	{
		...
		else
		{
			...
			if(blocks.size() > 0)
			{
				...
				worldObj.setBlock((int)Math.floor(posX), (int)Math.floor(posY), (int)Math.floor(posZ), Block.getBlockFromItem(disguiseStack.getItem()), disguiseStack.getMetadata(), 3);
			}
		}
	...
	@Override
	public float getExplosionResistance(Explosion explosion, World world, int x, int y, int z, Block block)
	{
