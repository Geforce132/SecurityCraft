...
public class BlockFakeWater extends BlockDynamicLiquid{
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand)
	{
		...
		{
			if (blockMaterial == Material.LAVA && world.getBlockState(pos.down()).getBlock().getMaterial(world.getBlockState(pos.down())) == Material.WATER)
			{
	...
	private int calculateFlowCost(World world, BlockPos pos, int distance, EnumFacing previousDirection)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (facing != previousDirection)
			{
				...
				if (!isBlocked(world, offsetPos, offsetState) && (offsetState.getBlock().getMaterial(offsetState) != blockMaterial || offsetState.getValue(LEVEL).intValue() > 0))
				{
