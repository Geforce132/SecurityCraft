...
public class ItemTaser extends Item {
	...
	public int findSlotMatchingUnusedItem(InventoryPlayer inventory, ItemStack stack)
	{
		for (int i = 0; i < inventory.mainInventory.size(); ++i)
		{
			...
			if (!inventory.mainInventory.get(i).isEmpty() && stackEqualExact(stack, inventory.mainInventory.get(i)) && !inventory.mainInventory.get(i).isItemDamaged() && !itemstack.isItemEnchanted() && !itemstack.hasDisplayName())
			{
				return i;
			}
		}
