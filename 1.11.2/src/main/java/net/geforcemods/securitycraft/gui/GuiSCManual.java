			...
			if(recipe != null)
			{
				for(int i = 0; i < 3; i++)
				{
					for(int j = 0; j < 3; j++){
						...
						if(recipe.get((i * 3) + j).getItem() instanceof ItemBlock)
							GuiUtils.drawItemStackToGui(mc, Block.getBlockFromItem(recipe.get((i * 3) + j).getItem()), (k + 100) + (j * 20), 144 + (i * 20), !(recipe.get((i * 3) + j).getItem() instanceof ItemBlock));
						else
							GuiUtils.drawItemStackToGui(mc, recipe.get((i * 3) + j).getItem(), recipe.get((i * 3) + j).getItemDamage(), (k + 100) + (j * 20), 144 + (i * 20), !(recipe.get((i * 3) + j).getItem() instanceof ItemBlock));
					}
				}
			}
			...
			for(HoverChecker chc : hoverCheckers)
			{
				if(chc != null && chc.checkHover(par1, par2))
				{
					if(chc instanceof StackHoverChecker && !((StackHoverChecker)chc).getStack().isEmpty())
						...
						drawHoveringText(((StringHoverChecker)chc).getName(), par1, par2);
				}
			}
		}
	}
			...
			outer:
				for(int i = 0; i < 3; i++)
				{
					for(int j = 0; j < 3; j++)
					{
						...
						if(!recipe.get((i * 3) + j).isEmpty())
							hoverCheckers.add(new StackHoverChecker(144 + (i * 20), 144 + (i * 20) + 16, (k + 100) + (j * 20), (k + 100) + (j * 20) + 16, 20, recipe.get((i * 3) + j)));
					}
				}
		}
