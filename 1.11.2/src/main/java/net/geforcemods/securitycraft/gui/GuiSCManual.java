	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks){
		...
		if(currentPage > -1){
			if(SecurityCraft.instance.manualPages.get(currentPage).getHelpInfo().equals("help.securitycraft:reinforced.info"))
				...
				fontRenderer.drawString(ClientUtils.localize(SecurityCraft.instance.manualPages.get(currentPage).getItem().getUnlocalizedName() + ".name"), startX + 39, 27, 0, false);
				...
				for(int i = 0; i < 3; i++)
				{
					for(int j = 0; j < 3; j++){
						...
						if(recipe.get((i * 3) + j).getItem() instanceof ItemBlock)
							GuiUtils.drawItemStackToGui(mc, Block.getBlockFromItem(recipe.get((i * 3) + j).getItem()), (startX + 100) + (j * 20), 144 + (i * 20), !(recipe.get((i * 3) + j).getItem() instanceof ItemBlock));
						else
							GuiUtils.drawItemStackToGui(mc, recipe.get((i * 3) + j).getItem(), recipe.get((i * 3) + j).getItemDamage(), (startX + 100) + (j * 20), 144 + (i * 20), !(recipe.get((i * 3) + j).getItem() instanceof ItemBlock));
					}
				}
			}
		...
		if(recipe != null && !reinforcedPage)
		{
			outer: for(int i = 0; i < 3; i++)
			{
				for(int j = 0; j < 3; j++)
				{
					...
					if(!recipe.get((i * 3) + j).isEmpty())
						hoverCheckers.add(new StackHoverChecker(144 + (i * 20), 144 + (i * 20) + 16, (startX + 100) + (j * 20), (startX + 100) + (j * 20) + 16, 20, recipe.get((i * 3) + j)));
				}
			}
		}
