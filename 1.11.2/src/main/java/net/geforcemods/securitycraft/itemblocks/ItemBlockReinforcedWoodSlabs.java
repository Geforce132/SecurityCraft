...
public class ItemBlockReinforcedWoodSlabs extends ItemBlock {
		...
		if(stack.getCount() == 0)
			...
			if(state.getBlock() == singleSlab){
				...
				if((side == EnumFacing.UP && half == BlockSlab.EnumBlockHalf.BOTTOM || side == EnumFacing.DOWN && half == BlockSlab.EnumBlockHalf.TOP) && value == type){
					IBlockState newState = makeState(valueProperty, value);
	...
	public boolean canPlaceBlockOnSide(World world, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack){
		...
		if(state.getBlock() == singleSlab){
			...
			if((side == EnumFacing.UP && !isTop || side == EnumFacing.DOWN && isTop) && object == state.getValue(valueProperty))
				return true;
		}

		pos = pos.offset(side);
		IBlockState updatedState = world.getBlockState(pos);
		return updatedState.getBlock() == singleSlab && object == updatedState.getValue(valueProperty) ? true : super.canPlaceBlockOnSide(world, originalPos, side, player, stack);
	}
