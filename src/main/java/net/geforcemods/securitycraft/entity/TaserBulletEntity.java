...
public class TaserBulletEntity extends ThrowableEntity {
	...
	public TaserBulletEntity(EntityType<TaserBulletEntity> type, World world){
	...
	protected void onImpact(RayTraceResult baseResult)
	{
		if(!world.isRemote)
		{
			if(baseResult.getType() == Type.ENTITY)
			{
				...
				{
					if(((LivingEntity) result.getEntity()).attackEntityFrom(CustomDamageSources.TASER, powered ? 2.0F : 1.0F))
					{
