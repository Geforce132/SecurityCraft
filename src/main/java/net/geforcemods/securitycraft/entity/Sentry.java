...
public class Sentry extends CreatureEntity implements IRangedAttackMob //needs to be a creature so it can target a player, ai is also only given to living entities
{
	private static final DataParameter<Owner> OWNER = EntityDataManager.<Owner> defineId(Sentry.class, Owner.getSerializer());
	private static final DataParameter<CompoundNBT> DISGUISE_MODULE = EntityDataManager.<CompoundNBT> defineId(Sentry.class, DataSerializers.COMPOUND_TAG);
	...
	public void setupSentry(PlayerEntity owner) {
		entityData.set(OWNER, new Owner(owner.getName().getColoredString(), PlayerEntity.createPlayerUUID(owner.getGameProfile()).toString()));
	...
	public void tick() {
		...
		if (!level.isClientSide) {
			...
			//no else if because animate can be changed in the above if statement
			if (animate) {
	...
	public boolean mobInteract(PlayerEntity player, Hand hand) {
		...
		if (getOwner().isOwner(player) && hand == Hand.MAIN_HAND) {
			...
			if (player.isCrouching())
				...
				item.useOn(new ItemUseContext(player, hand, new BlockRayTraceResult(new Vec3d(0.0D, 0.0D, 0.0D), Direction.NORTH, pos, false)));
				...
				String newOwner = player.getMainHandItem().getHoverName().getColoredString();
	...
	public void toggleMode(PlayerEntity player, int mode, boolean sendMessage) {
		...
		if (!player.level.isClientSide)
			SecurityCraft.channel.send(PacketDistributor.ALL.noArg(), new InitSentryAnimation(getCommandSenderBlockPosition(), true, SentryMode.values()[mode].isAggressive()));
	}
	...
	public void setTarget(LivingEntity target) {
		if (!getMode().isAggressive() && (target == null && previousTargetId != Long.MIN_VALUE || (target != null && previousTargetId != target.getId()))) {
			...
			animate = true;
			SecurityCraft.channel.send(PacketDistributor.ALL.noArg(), new InitSentryAnimation(getCommandSenderBlockPosition(), animate, animateUpwards));
		}
	...
	public void performRangedAttack(LivingEntity target, float distanceFactor) {
		...
		TileEntity te = level.getBlockEntity(getCommandSenderBlockPosition().below());
				...
				if (!stack.isEmpty()) {
					...
					if (dispenseBehavior instanceof ProjectileDispenseBehavior) {
						...
						if (throwableEntity instanceof AbstractArrowEntity)
							((AbstractArrowEntity) throwableEntity).setOwner(this);
		...
		double y = baseY - ((Entity) throwableEntity).getY();
		...
		if (shootSound == null)
			pdb.playSound(new ProxyBlockSource(level, getCommandSenderBlockPosition()));
		else
	...
	public void setDisguiseModule(ItemStack module) {
		...
		if (block != null) {
			...
			if (level.getBlockState(getCommandSenderBlockPosition()).isAir(level, getCommandSenderBlockPosition()))
				level.setBlockAndUpdate(getCommandSenderBlockPosition(), state.getShape(level, getCommandSenderBlockPosition()) == VoxelShapes.block() ? state : Blocks.AIR.defaultBlockState());
		}
