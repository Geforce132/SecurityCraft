...
public class EntityAITargetNearestPlayerOrMob extends EntityAINearestAttackableTarget<EntityLivingBase>
{
	...
	{
		List<EntityLivingBase> list = taskOwner.worldObj.<EntityLivingBase>getEntitiesWithinAABB(targetClass, taskOwner.getEntityBoundingBox().expand(getTargetDistance(), 4.0D, getTargetDistance()), targetEntitySelector);
		...
		else
		{
			...
			for(i = 0; i < list.size(); i++)
			{
				...
				if(potentialTarget instanceof EntityPlayer && !((EntityPlayer)potentialTarget).isSpectator() && !((EntityPlayer)potentialTarget).capabilities.isCreativeMode && !((EntitySentry)taskOwner).getOwner().isOwner(((EntityPlayer)potentialTarget)))
					break;
