...
public class FrameFeedHandler {
	...
	public static void onRenderFramePost(RenderFrameEvent.Post event) {
		...
		for (Entry<GlobalPos, CameraFeed> cameraView : activeFrameCameraFeeds.entrySet()) {
			...
			if (cameraPos.dimension().equals(level.dimension())) {
				...
				if (level.getBlockEntity(pos) instanceof SecurityCameraBlockEntity be) {
					...
					try {
						mc.gameRenderer.renderLevel(1.0F, 0L);
					}
					catch (Exception e) {
		...
		camera.setup(level, oldCamEntity == null ? player : oldCamEntity, !mc.options.getCameraType().isFirstPerson(), mc.options.getCameraType().isMirrored(), level.tickRateManager().isEntityFrozen(oldCamEntity) ? 1.0F : partialTick);
