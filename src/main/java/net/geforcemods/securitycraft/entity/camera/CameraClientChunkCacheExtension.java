...
// Modifications include simplifications, adaptation to NeoForge, MC version porting-related changes, and removals of code
// unused by SecurityCraft
...
public class CameraClientChunkCacheExtension {
	...
	public static Chunk replaceWithPacketData(WorldClient level, int x, int z, BiomeContainer biomeContainer, PacketBuffer packetData, NBTTagCompound chunkTag, int size, boolean fullChunk) {
		if (!Minecraft.getMinecraft().isCallingFromMinecraftThread())
		...
		Chunk chunk = CHUNK_MAP.get(longChunkPos);

		if (chunk == null) {
			Chunk newChunk = new Chunk(level, chunkPos, biomeContainer);
	...
	private static void modifyChunkMaps(Consumer<Long2ObjectOpenHashMap<Chunk>> operation) {
