...
public class CameraClientChunkCacheExtension {
	...
	public static void drop(ClientWorld level, ChunkPos chunkPos) {
		if (Minecraft.getInstance().isSameThread()) {
			...
			if (chunk != null) {
				...
				if (SecurityCraft.isASodiumModInstalled)
					SodiumCompat.onChunkStatusRemoved(chunkPos.x, chunkPos.z);
			}
		}
	}
	...
	public static Chunk replaceWithPacketData(ClientWorld level, int x, int z, BiomeContainer biomeContainer, PacketBuffer packetData, CompoundNBT chunkTag, int size, boolean fullChunk) {
		if (!Minecraft.getInstance().isSameThread())
