...
public class ReinforcedMovingPistonBlock extends BlockPistonMoving {
	public static TileEntity createTilePiston(IBlockState state, NBTTagCompound tag, EnumFacing facing, boolean extending, boolean shouldHeadBeRendered) {
		return new ReinforcedPistonBlockEntity(state, tag, facing, extending, shouldHeadBeRendered);
	}
	...
	public void onPlayerDestroy(World world, BlockPos pos, IBlockState state) {
		...
		if (oppositeState.getBlock() instanceof ReinforcedPistonBlock && oppositeState.getValue(BlockPistonBase.EXTENDED))
			world.setBlockToAir(oppositePos);
	}

	@Override
	public AxisAlignedBB getCollisionBoundingBox(IBlockState blockState, IBlockAccess world, BlockPos pos) {
		ReinforcedPistonBlockEntity tileEntityReinforcedPiston = getTilePistonAt(world, pos);
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		ReinforcedPistonBlockEntity tileEntityReinforcedPiston = getTilePistonAt(source, pos);

		return tileEntityReinforcedPiston != null ? tileEntityReinforcedPiston.getAABB(source, pos) : FULL_BLOCK_AABB;
	}

	private ReinforcedPistonBlockEntity getTilePistonAt(IBlockAccess world, BlockPos pos) {
