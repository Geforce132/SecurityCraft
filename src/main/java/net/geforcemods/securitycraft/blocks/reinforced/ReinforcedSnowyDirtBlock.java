...
public class ReinforcedSnowyDirtBlock extends SnowyDirtBlock implements IReinforcedBlock, IGrowable
{
	...
	{
		return this == SCContent.REINFORCED_GRASS_BLOCK.get() && world.getBlockState(pos.up()).isAir();
	}
			...
			while(true)
			{
				if(j >= i / 16)
				{
					...
					if(rand.nextInt(8) == 0)
					{
						...
						placeState = ((FlowersFeature)((DecoratedFeatureConfig)(flowers.get(0)).config).feature.feature).getRandomFlower(rand, tempPos);
					}
					else
						placeState = grass;
				...
				if(world.getBlockState(tempPos.down()).getBlock() != this || world.getBlockState(tempPos).func_224756_o(world, tempPos))
					break;
