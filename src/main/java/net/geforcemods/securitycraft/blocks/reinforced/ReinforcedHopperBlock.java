	...
	public static class ExtractionBlock implements IExtractionBlock {
		...
		public boolean canExtract(IOwnable te, World world, BlockPos pos, BlockState state) {
			...
			if (!hopperBe.getOwner().isValidated())
				return false;
			else if (!te.getOwner().owns(hopperBe)) {
				if (te instanceof IModuleInventory)
					return ((IModuleInventory) te).isAllowed(hopperBe.getOwner().getName()); //hoppers can extract out of e.g. chests if the hopper's owner is on the chest's allowlist module
