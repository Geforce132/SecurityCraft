			...
			for(Direction facing : Direction.values())
			{
				for(int i = 0; i < ConfigHandler.SERVER.laserBlockRange.get(); i++)
				{
					...
					if(offsetBlock == SCContent.LASER_BLOCK.get() && !offsetState.get(LaserBlock.POWERED))
					{
						...
						if(te instanceof IModuleInventory && ((IModuleInventory)te).hasModule(ModuleType.ALLOWLIST) && ModuleUtils.getPlayersFromModule(world, offsetPos, ModuleType.ALLOWLIST).contains(((LivingEntity) entity).getName().getString().toLowerCase()))
							return;
