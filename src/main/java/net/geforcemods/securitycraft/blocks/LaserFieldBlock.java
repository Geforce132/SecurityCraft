			...
			for(Direction facing : Direction.values())
			{
				for(int i = 0; i < ConfigHandler.CONFIG.laserBlockRange.get(); i++)
				{
					...
					if(block == SCContent.LASER_BLOCK.get() && !BlockUtils.getBlockProperty(world, offsetPos, LaserBlock.POWERED))
					{
						...
						if(te instanceof CustomizableTileEntity && ((CustomizableTileEntity)te).hasModule(ModuleType.WHITELIST) && ModuleUtils.getPlayersFromModule(world, offsetPos, ModuleType.WHITELIST).contains(((LivingEntity) entity).getName().getFormattedText().toLowerCase()))
							return;
						...
						if(te instanceof CustomizableTileEntity && ((CustomizableTileEntity)te).hasModule(ModuleType.HARMING))
						{
