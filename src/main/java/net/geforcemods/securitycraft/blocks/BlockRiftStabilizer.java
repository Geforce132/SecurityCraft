...
public class BlockRiftStabilizer extends BlockDisguisable {
	...
	public BlockRiftStabilizer(Material material) {
		super(material);
			...
			if (riftStabilizer.getOwner().isOwner(player)) {
				if (!world.isRemote) {
					if (riftStabilizer.isDisabled())
						...
						player.openGui(SecurityCraft.instance, GuiHandler.RIFT_STABILIZER, world, pos.getX(), pos.getY(), pos.getZ());
				}
	...
	public int getWeakPower(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing side) {
		...
		return state.getValue(POWERED) && state.getValue(HALF) == EnumDoorHalf.LOWER && te instanceof TileEntityRiftStabilizer ? 15 - (int) ((TileEntityRiftStabilizer) te).getLastTeleportDistance() : 0;
	}
	...
	public int getComparatorInputOverride(IBlockState state, World world, BlockPos pos) {
		...
		if (state.getValue(HALF) == EnumDoorHalf.LOWER && te instanceof TileEntityRiftStabilizer) {
			TileEntityRiftStabilizer riftStabilizer = ((TileEntityRiftStabilizer) te);

			return riftStabilizer.isModuleEnabled(EnumModuleType.REDSTONE) && riftStabilizer.getLastTeleportationType() != null ? riftStabilizer.getComparatorOutputFunction().applyAsInt(riftStabilizer.getLastTeleportationType()) : 0;
		}

		return 0;
	}

	public static TileEntityRiftStabilizer getConnectedTileEntity(World level, BlockPos pos) {
		...
		return te instanceof TileEntityRiftStabilizer ? ((TileEntityRiftStabilizer) te) : null;
	}
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		return new TileEntityRiftStabilizer();
	}
}
