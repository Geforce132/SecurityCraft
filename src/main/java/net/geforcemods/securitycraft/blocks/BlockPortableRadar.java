...
public class BlockPortableRadar extends BlockContainer {
	...
	public static void togglePowerOutput(World world, BlockPos pos, boolean toggleOn) {
		if(toggleOn && !((Boolean) world.getBlockState(pos).getValue(POWERED)).booleanValue()){
		...
		}else if(!toggleOn && ((Boolean) world.getBlockState(pos).getValue(POWERED)).booleanValue()){
	...
	public int isProvidingWeakPower(IBlockAccess world, BlockPos pos, IBlockState state, EnumFacing side){
		if(((CustomizableSCTE)world.getTileEntity(pos)).hasModule(EnumCustomModules.REDSTONE) && ((Boolean) state.getValue(POWERED)).booleanValue())
			return 15;
		else
			return 0;
	}
