		...
		if (!world.isClientSide) {
			...
			if (isPlayer || (entity instanceof MobEntity && tileEntity.capturesMobs())) {
				...
				if (isPlayer && PlayerUtils.isPlayerOnline(ownerName))
					PlayerUtils.sendMessageToPlayer(PlayerUtils.getPlayerFromName(ownerName), Utils.localize(SCContent.CAGE_TRAP.get().getDescriptionId()), Utils.localize("messages.securitycraft:cageTrap.captured", entity.getName(), pos), TextFormatting.BLACK);
			}
		}
	}
