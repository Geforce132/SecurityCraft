...
public class KeypadBlastFurnaceBlock extends AbstractKeypadFurnaceBlock {
	...
	public void animateTick(BlockState state, World world, BlockPos pos, Random rand) {
		if (state.get(LIT) && getDisguisedStateOrDefault(state, world, pos).getBlock() == this) {
			...
			if (state.get(OPEN)) {
				...
				double xOffset = axis == Axis.X ? direction.getXOffset() * 0.32D : randomNumber;
				double yOffset = rand.nextDouble() * 9.0D / 16.0D;
				double zOffset = axis == Axis.Z ? direction.getZOffset() * 0.32D : randomNumber;
