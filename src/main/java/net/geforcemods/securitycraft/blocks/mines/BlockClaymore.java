...
public class BlockClaymore extends BlockContainer implements IExplosive {
	...
	{
		if (!world.isRemote && BlockUtils.hasBlockProperty(world, pos, BlockClaymore.DEACTIVATED) && !((Boolean) world.getBlockState(pos).getValue(BlockClaymore.DEACTIVATED)).booleanValue())
		{
	...
	{
		if ((EnumFacing) world.getBlockState(pos).getValue(FACING) == EnumFacing.NORTH)
			setBlockBounds(0.225F, 0.000F, 0.175F, 0.775F, 0.325F, 0.450F);
		else if ((EnumFacing) world.getBlockState(pos).getValue(FACING) == EnumFacing.SOUTH)
			setBlockBounds(0.225F, 0.000F, 0.550F, 0.775F, 0.325F, 0.825F);
		else if ((EnumFacing) world.getBlockState(pos).getValue(FACING) == EnumFacing.EAST)
