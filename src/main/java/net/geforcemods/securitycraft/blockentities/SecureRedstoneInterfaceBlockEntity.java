...
public class SecureRedstoneInterfaceBlockEntity extends DisguisableBlockEntity implements ITickableTileEntity {
	//@formatter:off
	...
	public void tick() {
		if (!level.isClientSide) {
			if (!tracked) {
				if (!isSender())
					level.setBlockAndUpdate(worldPosition, getBlockState().setValue(SecureRedstoneInterfaceBlock.SENDER, false));
	...
	public void showParticleTrail(Collection<ServerPlayerEntity> players, ServerWorld level, Vector3d senderPos, Vector3d receiverPos, Vector3f color) {
		Vector3d senderToReceiver = receiverPos.subtract(senderPos);
		double step = senderToReceiver.length() * 6;
		Vector3f particleDirection = new Vector3f(senderToReceiver.normalize().scale(0.01F));

		for (int i = 0; i < step; i++) {
			Vector3d particlePos = Utils.lerp(receiverPos, senderPos, i / step);

			for (ServerPlayerEntity player : players) {
				level.sendParticles(player, new InterfaceHighlightParticleOptions(color.x(), color.y(), color.z(), particleDirection.x(), particleDirection.y(), particleDirection.z(), 1.0F), false, particlePos.x, particlePos.y, particlePos.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
			}
		}
	}
