...
public class ReinforcedPistonBlockStructureHelper { //this class doesn't extend BlockPistonStructureHelper because the content of that whole class is private
	...
	public ReinforcedPistonBlockStructureHelper(World world, BlockPos pos, EnumFacing pistonFacing, boolean extending) {
		this.world = world;
		pistonPos = pos;
	...
	public boolean canMove() {
		...
		if (!ReinforcedPistonBlock.canPush(state, world, pistonPos, blockToMove, moveDirection, false, moveDirection)) {
		...
		if (state.getBlock().isAir(state, world, origin))
			...
			if (i + toMove.size() > 12)
				...
				while (block.isStickyBlock(state)) {
					...
					if (state.getBlock().isAir(state, world, blockpos) || !ReinforcedPistonBlock.canPush(state, world, pistonPos, blockpos, moveDirection, false, moveDirection.getOpposite()) || blockpos.equals(pistonPos))
						break;
