...
public final class AlarmSoundHandler {
	...
	public static void playSound(AlarmBlockEntity be, Level level, double x, double y, double z, SoundEvent sound, SoundSource source, float volume, float pitch, long seed) {
		...
		SimpleSoundInstance soundInstance = new SimpleSoundInstance(event.getSound(), event.getSource(), event.getNewVolume(), event.getNewPitch(), RandomSource.create(seed), x, y, z);
