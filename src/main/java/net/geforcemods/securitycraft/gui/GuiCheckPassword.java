...
public class GuiCheckPassword extends GuiContainer {
	...
	@Override
	protected void keyTyped(char charTyed, int keyCode) throws IOException {
		if(isValidChar(charTyed) && charTyed != ''){
			Minecraft.getMinecraft().thePlayer.playSound("random.click", 0.15F, 1.0F);
			currentString += charTyed;
			setTextboxCensoredText(keycodeTextbox, currentString);
			checkCode(currentString);
		}else if(isValidChar(charTyed) && charTyed == ''){
			Minecraft.getMinecraft().thePlayer.playSound("random.click", 0.15F, 1.0F);
			...
			super.keyTyped(charTyed, keyCode);
	}
