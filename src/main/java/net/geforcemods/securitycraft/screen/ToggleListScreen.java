...
public class ToggleListScreen<T> extends GuiScreen {
	...
	public ToggleListScreen(IToggleableEntries<T> te, ITextComponent title, ITextComponent scrollListTitle, ITextComponent noSmartModule, ITextComponent smartModule) {
		this.te = te;
		hasSmartModule = te instanceof IModuleInventory && ((IModuleInventory) te).isModuleEnabled(ModuleType.SMART);
		hasRedstoneModule = te instanceof IModuleInventory && ((IModuleInventory) te).isModuleEnabled(ModuleType.REDSTONE);
