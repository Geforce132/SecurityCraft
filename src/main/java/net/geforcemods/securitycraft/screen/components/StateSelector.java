...
public class StateSelector extends Screen implements IGuiEventListener, IContainerListener {
	...
	protected void init() {
		...
		previousPageButton = new ExtendedButton(xStart + 69, yStart + 125, 20, 20, "<", button -> turnPage(-1));
		nextPageButton = new ExtendedButton(xStart + 126, yStart + 125, 20, 20, ">", button -> turnPage(1));
	...
	public void render(int mouseX, int mouseY, float partialTick) {
		...
		blit(xStart, yStart, 0, 0, 193, 150);
		super.render(mouseX, mouseY, partialTick);
		previousPageButton.render(mouseX, mouseY, partialTick);
		...
		for (int i = 0; i < propertyButtons.size(); i++) {
			...
			font.draw(propertyName, xStart + 91 - font.width(propertyName) - 2, yStart + i * 23 + 10, 0x404040);
		}

		font.draw(page + "/" + amountOfPages, xStart + 100, yStart + 130, 0x404040);
	}
	...
	public void renderBlockModel(BlockState state, MatrixStack pose, IRenderTypeBuffer bufferSource) {
		if (state.getRenderShape() == BlockRenderType.MODEL) {
			...
			blockRenderer.getModelRenderer().renderModel(pose.last(), bufferSource.getBuffer(RenderTypeLookup.getRenderType(state)), state, blockModel, r, g, b, 0xF000F0, OverlayTexture.NO_OVERLAY, EmptyModelData.INSTANCE);
		}
	}
	...
	public class BlockStatePropertyButton<T extends Comparable<T>> extends ToggleComponentButton {
		...
		public BlockStatePropertyButton(int xPos, int yPos, int width, int height, int initialValue, IProperty<T> property) {
		...
		public void onValueChange() {
			if (property != null) {
				...
				setMessage(property.getName(value));
			}
		}
