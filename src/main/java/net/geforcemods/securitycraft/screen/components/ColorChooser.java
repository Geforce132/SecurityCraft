...
public class ColorChooser extends Screen implements GuiEventListener, NarratableEntry {
	...
	private final Component rText = new TextComponent("R");
	private final Component gText = new TextComponent("G");
	private final Component bText = new TextComponent("B");
	private final Component rgbHexText = new TextComponent("#");
	...
	protected void init() {
		...
		rBox = addRenderableWidget(new FixedEditBox(font, colorFieldRight + 13, colorFieldTop, 26, 10, rText));
		gBox = addRenderableWidget(new FixedEditBox(font, colorFieldRight + 13, colorFieldTop + 15, 26, 10, gText));
		bBox = addRenderableWidget(new FixedEditBox(font, colorFieldRight + 13, colorFieldTop + 30, 26, 10, bText));
		rgbHexBox = addRenderableWidget(new FixedEditBox(font, colorFieldRight + 13, colorFieldTop + 45, 46, 10, rgbHexText));
		rBox.setValue("" + red);
		gBox.setValue("" + green);
		bBox.setValue("" + blue);
	...
	public FixedEditBox getRgbHexBox() {
		return rgbHexBox;
	}

	class HueSlider extends ForgeSlider {
		public HueSlider(int x, int y, int width, int height, double currentValue) {
			super(x, y, width, height, TextComponent.EMPTY, TextComponent.EMPTY, 0.0D, 360.0D, currentValue, 1.0D, 0, false);
		}
