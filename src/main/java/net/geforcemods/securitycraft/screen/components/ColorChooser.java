...
public class ColorChooser extends Screen implements GuiEventListener, NarratableEntry {
	...
	protected void init() {
		...
		addRenderableWidget(hueSlider = new HueSlider(colorFieldLeft - 2, yStart + 85, 82, 20, h * 360.0D, slider -> {
	...
	public void render(PoseStack pose, int mouseX, int mouseY, float partialTick) {
		if (!disabled) {
			...
			blit(pose, (int) selectionX - 1, (int) selectionY - 1, colorFieldHoverChecker.checkHover(mouseX, mouseY, getFocused()) ? 148 : 145, 20, 3, 3); //color field indicator
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (!disabled) {
			...
			clickedInDragRegion = colorFieldHoverChecker.checkHover(mouseX, mouseY, getFocused());
	...
	public class HueSlider extends Slider {
		public HueSlider(int x, int y, int width, int height, double currentValue, ISlider onChangeSliderValue) {
			super(x, y, width, height, TextComponent.EMPTY, TextComponent.EMPTY, 0.0D, 360.0D, currentValue, false, false, b -> {}, onChangeSliderValue);
		}
