...
public class AlarmOptionsScreen extends Screen {
	...
	protected void init() {
		...
		plusSecond = addRenderableWidget(new ExtendedButton(buttonsX + 116, buttonY, 32, buttonHeight, new TranslatableComponent("gui.securitycraft:alarm.plus_one_second"), b -> changeSoundLength(1)));
		plusTenSeconds = addRenderableWidget(new ExtendedButton(buttonsX + 150, buttonY, 32, buttonHeight, new TranslatableComponent("gui.securitycraft:alarm.plus_ten_seconds"), b -> changeSoundLength(10)));
		plusMinute = addRenderableWidget(new ExtendedButton(buttonsX + 184, buttonY, 32, buttonHeight, new TranslatableComponent("gui.securitycraft:alarm.plus_one_minute"), b -> changeSoundLength(60)));
		setSoundLength(alarmScreen.soundLength);
		addRenderableWidget(new CallbackSlider(leftPos + imageWidth / 2 - 50, buttonY + 25, 100, 20, Utils.localize("gui.securitycraft:alarm.pitch", ""), TextComponent.EMPTY, 0.5D, 2.0D, alarmScreen.pitch, 0.05D, 0, true, slider -> alarmScreen.pitch = (float) slider.getValue())).setFGColor(0xE0E0E0);
	}
