...
public interface IPasscodeProtected extends ICodebreakable {
	...
	default boolean verifyPasscodeSet(Level level, BlockPos pos, IOwnable ownable, Player player) {
		if (!level.isClientSide) {
			...
			if (ownable.isOwnedBy(player))
				...
				PlayerUtils.sendMessageToPlayer(player, new TextComponent("SecurityCraft"), Utils.localize("messages.securitycraft:passcodeProtected.notSetUp"), ChatFormatting.DARK_RED);
		}
	...
	/**
	 * Fills the given CompoundTag with the salt key and passcode.
	 *
