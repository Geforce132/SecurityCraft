 ...
 * {@link ICodebreakable} as most passcode-protected object are likely able to be hackable using the Codebreaker by default.
 *
 * @author Geforce
 */
public interface IPasscodeProtected extends ICodebreakable {
	 ...
	 *
	 * @param world The level of this object
	...
	default boolean verifyPasscodeSet(World world, BlockPos pos, IOwnable ownable, EntityPlayer player) {
		if (!world.isRemote) {
			...
			if (ownable.isOwnedBy(player))
				openSetPasscodeScreen((EntityPlayerMP) player, world, pos);
			else
	 ...
	 * @param world The world to open the screen in
	 * @param pos The position to open the screen at
	 */
	default void openSetPasscodeScreen(EntityPlayerMP player, World world, BlockPos pos) {
		player.openGui(SecurityCraft.instance, Screens.SETUP_PASSCODE.ordinal(), world, pos.getX(), pos.getY(), pos.getZ());
	}

	@Override
	default boolean shouldAttemptCodebreak(EntityPlayer player) {
		if (getPasscode() == null) {
			PlayerUtils.sendMessageToPlayer(player, new TextComponentString("SecurityCraft"), Utils.localize("messages.securitycraft:passcodeProtected.notSetUp"), TextFormatting.DARK_RED);
			return false;
		}
	...
	@Override
	public default void useCodebreaker(EntityPlayer player) {
		activate(player);
	}
	 ...
	 */
	default void savePasscodeAndSalt(NBTTagCompound tag) {
		if (getSaltKey() != null) {
			tag.setUniqueId("saltKey", getSaltKey());

			if (shouldSaveSalt()) { //This is only used for operator interactions, like using CTRL-pick-block or structure blocks
				tag.setString("salt", PasscodeUtils.bytesToString(getSalt()));
				setSaveSalt(false);
			}
		}

		if (getPasscode() != null)
			tag.setString("passcode", PasscodeUtils.bytesToString(getPasscode()));
	}
	...
	default void loadSaltKey(NBTTagCompound tag) {
		...
		if (passcode.length() == 32) {
			if (!SaltData.containsKey(saltKey)) { //If the passcode hash is set correctly, but no salt key or no salt associated with the given key can be found:
				if (tag.hasKey("salt")) //If the tag also has info on the salt that was used for hashing, register this salt under a new salt key
