...
public class ItemBlockReinforcedSlabs extends ItemBlock {
	...
	public boolean canPlaceBlockOnSide(World world, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack){
		...
		if(state.getBlock() == singleSlab){
			...
			if((side == EnumFacing.UP && !isTop || side == EnumFacing.DOWN && isTop) && type == state.getValue(iproperty))
				return true;
		}

		pos = pos.offset(side);
		IBlockState updatedState = world.getBlockState(pos);
		return updatedState.getBlock() == singleSlab && type == updatedState.getValue(iproperty) ? true : super.canPlaceBlockOnSide(world, originalPos, side, player, stack);
	}
