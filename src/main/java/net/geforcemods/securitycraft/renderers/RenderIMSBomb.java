...
public class RenderIMSBomb extends Render<EntityIMSBomb> {
	...
	@Override
	public void doRender(EntityIMSBomb entity, double x, double y, double z, float entityYaw, float partialTicks) {
		GlStateManager.pushMatrix();

		GlStateManager.translate((float)x - 0.1F, (float)y, (float)z - 0.1F);
		bindEntityTexture(entity);
		GlStateManager.scale(1.4F, 1.4F, 1.4F);
		modelBomb.render(entity, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);

		GlStateManager.popMatrix();
	}

	@Override
	protected ResourceLocation getEntityTexture(EntityIMSBomb entity) {
		return TEXTURE;
	}
