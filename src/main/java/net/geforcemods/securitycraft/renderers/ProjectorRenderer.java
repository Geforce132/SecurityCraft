...
public class ProjectorRenderer extends TileEntitySpecialRenderer<ProjectorBlockEntity> {
	...
	public void render(ProjectorBlockEntity te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		...
		if (te.isActive() && !te.isEmpty()) {
			...
			if (!state.getPropertyKeys().contains(ProjectorBlock.FACING) || !state.getPropertyKeys().contains(ProjectorBlock.HANGING))
				return;

			BlockRendererDispatcher blockRendererDispatcher = Minecraft.getMinecraft().getBlockRendererDispatcher();
			EnumFacing facing = state.getValue(ProjectorBlock.FACING);
			...
			for (int fakeX = 0; fakeX < te.getProjectionWidth(); fakeX++) {
				for (int fakeY = 0; yLoopBoundary.test(te, hanging, fakeY); fakeY = hanging ? fakeY - 1 : fakeY + 1) {
					...
					if (pos != null && (te.isOverridingBlocks() || te.getWorld().isAirBlock(pos))) {
						...
						BlockEntityRenderDelegate.PROJECTOR.tryRenderDelegate(te, 0, 0, -1, partialTicks, destroyStage, alpha);
