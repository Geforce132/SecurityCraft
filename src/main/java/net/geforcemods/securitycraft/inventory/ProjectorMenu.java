...
public class ProjectorMenu extends StateSelectorAccessMenu {
	...
	private final ProjectorBlockEntity te;
	private Slot projectedBlockSlot;

	public ProjectorMenu(InventoryPlayer inventory, ProjectorBlockEntity te) {
		this.te = te;

		// A custom slot that prevents non-Block items from being inserted into the projector
		projectedBlockSlot = addSlotToContainer(new Slot(new BlockEntityInventoryWrapper<>(te, this), 36, 80, 23) {
			@Override
