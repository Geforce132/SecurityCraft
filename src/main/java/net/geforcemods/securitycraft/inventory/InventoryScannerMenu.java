...
public class InventoryScannerMenu extends Container {
	...
	public InventoryScannerMenu(int windowId, World world, BlockPos pos, PlayerInventory inventory) {
		...
		if (te.isOwner(inventory.player) && te.isModuleEnabled(ModuleType.STORAGE)) {
			for (int i = 0; i < 9; i++) {
				for (int j = 0; j < 3; j++) {
	...
	public void removed(PlayerEntity player) {
		...
		Utils.setISinTEAppropriately(player.level, te.getBlockPos(), ((InventoryScannerBlockEntity) player.level.getBlockEntity(te.getBlockPos())).getContents());
	}
