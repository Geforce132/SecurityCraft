...
public class SCEventHandler {
			...
			PLAYING_TUNES.forEach((player, pair) -> {
				...
				if (ticksRemaining == 0) {
					...
					if (note != null) {
						...
						player.level.playSound(null, player.getCommandSenderBlockPosition(), sound, SoundCategory.RECORDS, 3.0F, pitch);
						handlePlayedNote(player.level, player.getCommandSenderBlockPosition(), note.noteID, note.instrumentName);
	...
	public static void onDamageTaken(LivingHurtEvent event) {
		...
		if (event.getSource() == CustomDamageSources.ELECTRICITY)
			world.playSound(null, entity.getCommandSenderBlockPosition(), SCSounds.ELECTRIFIED.event, SoundCategory.BLOCKS, 0.25F, 1.0F);
	...
	public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
		...
		if (!world.isClientSide) {
			...
			if (te instanceof INameSetter && (te instanceof SecurityCameraBlockEntity || te instanceof PortableRadarBlockEntity) && PlayerUtils.isHoldingItem(event.getPlayer(), Items.NAME_TAG, event.getHand()) && event.getPlayer().getItemInHand(event.getHand()).hasCustomHoverName()) {
				...
				if (nametag.getHoverName().equals(nameable.getCustomName())) {
					PlayerUtils.sendMessageToPlayer(event.getPlayer(), Utils.localize(te.getBlockState().getBlock().getDescriptionId()), Utils.localize("messages.securitycraft:naming.alreadyMatches", nameable.getCustomName()), TextFormatting.RED);
					return;
				}
				...
				PlayerUtils.sendMessageToPlayer(event.getPlayer(), Utils.localize(te.getBlockState().getBlock().getDescriptionId()), Utils.localize("messages.securitycraft:naming.named", nameable.getCustomName()), TextFormatting.RED);
				return;
			}
		}
	...
	public static void onOwnership(OwnershipEvent event) {
		...
		if (te instanceof IOwnable) {
			String name = event.getPlayer().getName().getColoredString();
		...
		if (tileEntity instanceof IPasswordProtected && ((IPasswordProtected) tileEntity).isCodebreakable()) {
			if (ConfigHandler.SERVER.allowCodebreakerItem.get()) {
				...
				if (event.getPlayer().isCreative() || new Random().nextInt(3) == 1)
					...
					PlayerUtils.sendMessageToPlayer(event.getPlayer(), Utils.localize(SCContent.CODEBREAKER.get().getDescriptionId()), Utils.localize("messages.securitycraft:codebreaker.failed"), TextFormatting.RED);
					return true;
				}
			}
			else {
