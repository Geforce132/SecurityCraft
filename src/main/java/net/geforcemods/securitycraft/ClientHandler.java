...
public class ClientHandler {
	...
	public static void onModelRegistry(ModelRegistryEvent event) {
		ModelLoader.addSpecialModel(new ResourceLocation(SecurityCraft.MODID, "block/secure_redstone_interface_sender_on"));
		ModelLoader.addSpecialModel(new ResourceLocation(SecurityCraft.MODID, "block/secure_redstone_interface_receiver_on"));
	}
	...
	public static void onModelBake(ModelBakeEvent event) {
		...
		ResourceLocation modelBase = new ResourceLocation(sriName.getNamespace(), "block/" + sriName.getPath());
		ResourceLocation poweredSriSenderLocation = new ResourceLocation(modelBase.getNamespace(), modelBase.getPath() + "_sender_on");
		ResourceLocation poweredSriReceiverLocation = new ResourceLocation(modelBase.getNamespace(), modelBase.getPath() + "_receiver_on");
	...
	private static void registerDisguisedModel(Map<ResourceLocation, IBakedModel> modelRegistry, BlockState state, Function<IBakedModel, IDynamicBakedModel> modelFunction) {
	...
	private static IBakedModel bakeDirectionalModel(ResourceLocation location, Direction facing) {
		...
		ItemModelsProperties.register(SCContent.SONIC_SECURITY_SYSTEM_ITEM.get(), LINKING_STATE_PROPERTY, (stack, level, entity) -> {
			...
			float linkingState = getLinkingState(level, player, stack, (_level, pos) -> {
				...
				if (!(te instanceof IOwnable) || !((IOwnable) te).isOwnedBy(player)) {
					if (IDisguisable.getDisguisedBlockState(te).isPresent())
						return false;
				}
	...
	public static void refreshModelData(TileEntity be) {
		...
		ModelDataManager.getModelData(level, pos); //Actually calculates and applies the new model data that was requested before
	...
	public static void putDisguisedBeRenderer(TileEntity disguisableBlockEntity, ItemStack stack) {
		DISGUISED_BLOCK_RENDER_DELEGATE.putDelegateFor(disguisableBlockEntity, NBTUtil.readBlockState(stack.getOrCreateTag().getCompound("SavedState")), new ModuleItemContainer(stack).getItem(0).copy());
	}
	...
	private static IModelTransform modelRotationFromDirection(Direction direction) {
		switch (direction) {
			case DOWN:
