...
public class AdminToolItem extends Item {
	...
	public ActionResultType onItemUseFirst(ItemStack stack, ItemUseContext ctx) {
		...
		TranslationTextComponent adminToolName = Utils.localize(getDescriptionId());

		if (!world.isClientSide && ConfigHandler.SERVER.allowAdminTool.get()) {
			...
			if (te != null) {
				...
				if (te instanceof SecretSignBlockEntity) {
					...
					for (int i = 0; i < 4; i++) {
						PlayerUtils.sendMessageToPlayer(player, adminToolName, ((SecretSignBlockEntity) te).messages[i], TextFormatting.DARK_PURPLE);
					}
