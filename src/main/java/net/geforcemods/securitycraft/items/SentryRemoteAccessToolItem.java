		...
		if(!world.isRemote){
			if(!sentries.isEmpty()) {
				...
				if(!isSentryAdded(stack, world, pos2)){
					...
					if(availSlot == 0){
						PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.REMOVE_ACCESS_SENTRY.get().getTranslationKey()), ClientUtils.localize("messages.securitycraft:srat.noSlots"), TextFormatting.RED);
						return ActionResultType.FAIL;
					}

					if(world.getTileEntity(pos2) instanceof IOwnable && !((IOwnable) world.getTileEntity(pos2)).getOwner().isOwner(player)){
						PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.REMOVE_ACCESS_SENTRY.get().getTranslationKey()), ClientUtils.localize("messages.securitycraft:srat.cantBind"), TextFormatting.RED);
						return ActionResultType.FAIL;
					}
					...
					PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.REMOVE_ACCESS_SENTRY.get().getTranslationKey()), ClientUtils.localize("messages.securitycraft:srat.bound").replace("#", Utils.getFormattedCoordinates(pos2)), TextFormatting.GREEN);
				}else{
					removeTagFromItemAndUpdate(stack, pos2, player);
					PlayerUtils.sendMessageToPlayer(player, ClientUtils.localize(SCContent.REMOVE_ACCESS_SENTRY.get().getTranslationKey()), ClientUtils.localize("messages.securitycraft:srat.unbound").replace("#", Utils.getFormattedCoordinates(pos2)), TextFormatting.RED);
				}
			}
		}
