...
public class ReinforcedDoorItem extends ItemBlock {
		...
		if (facing != EnumFacing.UP)
			...
			if (player.canPlayerEdit(pos, facing, stack) && getBlock().canPlaceBlockAt(world, pos)) {
				...
				if (world.getTileEntity(pos) != null) {
					((OwnableBlockEntity) world.getTileEntity(pos)).setOwner(player.getGameProfile().getId().toString(), player.getName());
					((OwnableBlockEntity) world.getTileEntity(pos.up())).setOwner(player.getGameProfile().getId().toString(), player.getName());
				}
	...
	public static void placeDoor(World world, BlockPos pos, EnumFacing facing, Block door, boolean isRightHinge) //naming might not be entirely correct, but it's giving a rough idea
	{
		...
		boolean isAnyPowered = BlockUtils.hasActiveSCBlockNextTo(world, pos) || BlockUtils.hasActiveSCBlockNextTo(world, blockAbove);
