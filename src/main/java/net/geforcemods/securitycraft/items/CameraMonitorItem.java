	...
	public static List<Pair<GlobalPos, String>> getCameraPositions(CompoundTag tag) {
		...
		for (int i = 1; i <= 30; i++) {
			if (tag != null && tag.contains("Camera" + i)) {
				...
				list.add(Pair.of(GlobalPos.of(coords.length == 4 ? ResourceKey.create(Registry.DIMENSION_REGISTRY, new ResourceLocation(coords[3])) : Level.OVERWORLD, pos), cameraName));
			}
			else
