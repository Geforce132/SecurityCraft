	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ){
		if(!world.isRemote){
			IPasswordConvertible.BLOCKS.forEach((pc) -> {
				if(world.getBlock(x, y, z) == ((IPasswordConvertible)pc).getOriginalBlock())
				{
					if(((IPasswordConvertible)pc).convert(player, world, x, y, z) && !player.capabilities.isCreativeMode)
						stack.stackSize--;
