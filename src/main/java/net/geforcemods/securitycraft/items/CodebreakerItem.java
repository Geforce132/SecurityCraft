...
public class CodebreakerItem extends Item {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
		...
		if (hand == Hand.MAIN_HAND && player.getHeldItemOffhand().getItem() == SCContent.BRIEFCASE.get()) {
			if(!world.isRemote && !ConfigHandler.SERVER.allowCodebreakerItem.get()) {
				...
				return ActionResult.newResult(ActionResultType.FAIL, codebreaker);
			}
			else {
		...
		return ActionResult.newResult(ActionResultType.PASS, codebreaker);
	}
