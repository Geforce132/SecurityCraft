...
- Change: If JustEnoughItems is installed, buttons from the Universal Block Modifier's menu will appear on the left instead of the right
...
- Fix: Taser bullet has incorrect velocity which leads to unexpected shooting behavior
- Fix: Incorrect Reinforced Stained Glass Pane recipes
...
- Fix: Reinforced Doors can be opened by anybody's Keypad/Scanner/etc.
- Fix: Torches, Doors, etc. cannot be placed on Keypads
- Fix: NEI crashes when searching for specific strings due to incorrectly registered item models (Thanks Firmanator19!)
...
- Fix: Reinforced Iron Fence Gate is not craftable
- Fix: Disguised Keypad doesn't respect biome colors (Gray grass etc.)
...
- Fix: Bouncing Betty can explode while it is defused
...
- Fix: Unable to add players to modules
...
- Fix: North-facing camera views being able to rotate backwards when turning left, and not being able to turn right after previously turning left
- Fix: Reinforced Doors can be activated by normal redstone
...
- New: Updated to Forge v11.14.3.1556
