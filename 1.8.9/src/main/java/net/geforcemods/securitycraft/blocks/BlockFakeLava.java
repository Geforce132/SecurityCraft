...
public class BlockFakeLava extends BlockDynamicLiquid implements ITileEntityProvider {
		...
		if (level > 0)
		{
			...
			if (adjacentSourceBlocks >= 2 && blockMaterial == Material.water)
			{
				...
				if (stateBelow.getBlock().getMaterial().isSolid())
					nextLevel = 0;
				else if (stateBelow.getBlock().getMaterial() == blockMaterial && stateBelow.getValue(LEVEL).intValue() == 0)
					nextLevel = 0;
			}
	...
	private int calculateFlowCost(World worldIn, BlockPos pos, int distance, EnumFacing previousDirection)
	{
		int cost = 1000;
		...
		while (iterator.hasNext())
		{
			...
			if (facing != previousDirection)
			{
				...
				if (!isBlocked(worldIn, offsetPos, offsetState) && (offsetState.getBlock().getMaterial() != blockMaterial || offsetState.getValue(LEVEL).intValue() > 0))
				{
					if (!isBlocked(worldIn, offsetPos.down(), offsetState))
						return distance;

					if (distance < 4)
					{
						int oppositeCost = calculateFlowCost(worldIn, offsetPos, distance + 1, facing.getOpposite());
	...
	private Set<?> getPossibleFlowDirections(World world, BlockPos pos)
	{
		...
		while (iterator.hasNext())
		{
			...
			if (!isBlocked(world, offsetPos, offsetState) && (offsetState.getBlock().getMaterial() != blockMaterial || offsetState.getValue(LEVEL).intValue() > 0))
			{
				int oppositeCost;
	...
	@Override
	public void onBlockAdded(World worln, BlockPos pos, IBlockState state)
	{
		if (!checkForMixing(worln, pos, state))
			worln.scheduleUpdate(pos, this, tickRate(worln));
	}
