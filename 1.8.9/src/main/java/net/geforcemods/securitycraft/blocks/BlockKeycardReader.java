	...
	public void insertCard(World world, BlockPos pos, ItemStack stack, EntityPlayer player) {
		...
		if(!world.isRemote)
		{
			if(requiredLevel != -1 && ((TileEntityKeycardReader)world.getTileEntity(pos)).doesRequireExactKeycard() && requiredLevel != cardLvl)
				PlayerUtils.sendMessageToPlayer(player, StatCollector.translateToLocal("tile.securitycraft:keycardReader.name"), StatCollector.translateToLocal("messages.securitycraft:keycardReader.required").replace("#r", ((IPasswordProtected) world.getTileEntity(pos)).getPassword()).replace("#c", "" + cardLvl), EnumChatFormatting.RED);
