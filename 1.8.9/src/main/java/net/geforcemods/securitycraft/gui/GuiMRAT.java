...
public class GuiMRAT extends GuiContainer{
	...
	private static final ResourceLocation field_110410_t = new ResourceLocation("securitycraft:textures/gui/container/mrat.png");
	...
	{
		fontRendererObj.drawString(StatCollector.translateToLocal("item.securitycraft:remoteAccessMine.name"), xSize / 2 - fontRendererObj.getStringWidth(StatCollector.translateToLocal("item.securitycraft:remoteAccessMine.name")), -25 + 13, 0xFF0000);
		...
		for(int i = 0; i < 6; i++)
		{
			...
			if(coords[0] == 0 && coords[1] == 0 && coords[2] == 0)
				line = StatCollector.translateToLocal("gui.securitycraft:mrat.notBound");
			else
				line = StatCollector.translateToLocal("gui.securitycraft:mrat.mineLocations").replace("#location", Utils.getFormattedCoordinates(new BlockPos(coords[0], coords[1], coords[2])));
