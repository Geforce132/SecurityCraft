	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ){
		if(stack.stackSize == 0)
			...
			if(state.getBlock() == singleSlab){
				IProperty variantProprety = singleSlab.getVariantProperty();
				Comparable value = state.getValue(variantProprety);
				...
				if(world.getTileEntity(pos) instanceof IOwnable){
					...
					if(!owner.owns((IOwnable) world.getTileEntity(pos))){
						if(!world.isRemote)
				...
				if((side == EnumFacing.UP && half == BlockSlab.EnumBlockHalf.BOTTOM || side == EnumFacing.DOWN && half == BlockSlab.EnumBlockHalf.TOP) && value == variant){
					IBlockState doubleState = doubleSlab.getDefaultState().withProperty(variantProprety, value);
					...
					if(world.checkNoEntityCollision(doubleSlab.getCollisionBoundingBox(world, pos, doubleState)) && world.setBlockState(pos, doubleState, 3)){
						...
						if(owner != null)
							((IOwnable) world.getTileEntity(pos)).getOwner().set(owner);
					}
	...
	private boolean tryPlace(ItemStack stack, World world, BlockPos pos, Object variantInStack){
		...
		if(state.getBlock() == singleSlab){
			...
			if(value == variantInStack){
				IBlockState stateWithProperty = doubleSlab.getDefaultState().withProperty((IProperty) singleSlab.getVariantProperty(), value);
