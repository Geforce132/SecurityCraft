	...
	public void drawScreen(int par1, int par2, float par3){
		...
		if(currentPage > -1){
			if(mod_SecurityCraft.instance.manualPages.get(currentPage).getHelpInfo().equals("help.reinforced.info"))
				...
				fontRenderer.drawString(ClientUtils.localize(mod_SecurityCraft.instance.manualPages.get(currentPage).getItem().getUnlocalizedName() + ".name"), k + 39, 27, 0, false);

			fontRenderer.drawSplitString(ClientUtils.localize(mod_SecurityCraft.instance.manualPages.get(currentPage).getHelpInfo()), k + 18, 45, 225, 0);
		}else{
