...
public class GuiSCManual extends GuiScreen {
	...
	@Override
	public void drawScreen(int mosueX, int mouseY, float partialTicks){
		...
		if(currentPage > -1){
			if(SecurityCraft.instance.manualPages.get(currentPage).getHelpInfo().equals("help.securitycraft:reinforced.info"))
				fontRendererObj.drawString(ClientUtils.localize("gui.securitycraft:scManual.reinforced"), startX + 39, 27, 0, false);
			else
			...
			if(I18n.hasKey("gui.securitycraft:scManual.author"))
				fontRendererObj.drawString(ClientUtils.localize("gui.securitycraft:scManual.author"), startX + 65, 170, 0, false);
		}

		for(int i = 0; i < buttonList.size(); i++)
			buttonList.get(i).drawButton(mc, mosueX, mouseY);
		...
		if(currentPage > -1){
			...
			{
				if(chc != null && chc.checkHover(mosueX, mouseY))
				{
					if(chc instanceof StackHoverChecker && ((StackHoverChecker)chc).getStack() != null)
						renderToolTip(((StackHoverChecker)chc).getStack(), mosueX, mouseY);
					else if(chc instanceof StringHoverChecker && ((StringHoverChecker)chc).getName() != null)
						drawHoveringText(mc.fontRendererObj.listFormattedStringToWidth(((StringHoverChecker)chc).getName(), 220), mosueX, mouseY, mc.fontRendererObj);
				}
			}
		}
	}
