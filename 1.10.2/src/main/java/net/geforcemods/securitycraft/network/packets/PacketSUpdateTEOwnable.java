...
public class PacketSUpdateTEOwnable implements IMessage
{
	...
	public static class Handler implements IMessageHandler<PacketSUpdateTEOwnable, IMessage>
	{
		...
		{
			Minecraft.getMinecraft().addScheduledTask(() -> ((TileEntityOwnable)Minecraft.getMinecraft().theWorld.getTileEntity(message.pos)).setOwner(message.uuid, message.name));
			return null;
		}
	}
}
